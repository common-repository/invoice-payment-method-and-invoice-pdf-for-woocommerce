...
$af_ips_invoice_suffix_or_prefix       = get_option( 'af_ips_invoice_suffix_or_prefix' );
?>
<!DOCTYPE html>
<html>
<head>
	...
	<div class="af-pdf-wrapper" >
		<section class="af-pdf-header" >
			<div class="af-invoice-info">
				...
				<div class="af-invoice-info-content">
					<h1><?php esc_html_e( 'INVOICE', 'af_ips_invoices' ); ?></h1>
					<ul>
						<li><p><label><?php esc_html_e( 'Invoice Name', 'af_ips_invoices' ); ?></label><span><?php echo esc_attr( $af_invoice_number ); ?></span></p></li>
		...
		<section class="af-pdf-content" >
			<table>
				<thead>
						<?php if ( $af_imge ) { ?>
						<th><?php echo esc_html__( 'Product Image', 'af_ips_invoices' ); ?></th>		 
						<?php } ?>
				...
				<tbody>
					...
					foreach ( $order->get_items() as $item_id => $item ) {
						...
						$af_li_total    = $order->get_line_total( $item );
						?>
						<tr>
							<?php if ( $af_imge ) { ?>
							...
							<?php } ?>
							<?php if ( $af_line_total ) { ?>
							<td><?php echo wp_kses_post( wc_price( $af_li_total ) ); ?></td>
							<?php } ?>
							<?php if ( $af_toatl_inc_taxt ) { ?>
		...
		<section class="af-pdf-footer">
			...
			<ul>
				<li><p><label><?php echo esc_html__( 'Customer Name:', 'af_ips_invoices' ); ?></label> <span><?php echo esc_attr( $invoice_billing_first_name ); ?></span></p></li>
				<li><p><label><?php echo esc_html__( 'Postcode / ZIP:', 'af_ips_invoices' ); ?></label><span><?php echo esc_attr( $billing_postcode ); ?></span></p></li>
				<li><p><label><?php esc_html_e( 'Shipping Country:', 'af_ips_invoices' ); ?></label><span><?php echo esc_attr( $shipping_country ); ?></span></p></li>
